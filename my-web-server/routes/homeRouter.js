...
router.get('/', async (req, res) => {
  try {
    ...
    // Tính tổng tiền `tienHuy` của các đơn "Đã hủy"
    const tongTienHuy = HoaDonhuy.reduce((total, booking) => {
      return total + (booking.tienHuy || 0); // Đảm bảo không có lỗi nếu `tienHuy` bị null hoặc undefined
    }, 0);
    ...
    res.render('home', {
      ...
      tonghdcxnhuy,
      tonghdcvp,
      tienViPham: tongTienHuy, // Tổng tiền của các đơn "Đã hủy"
      bookingsToday,
      bookingsTodayTong
    });
  } catch (error) {
