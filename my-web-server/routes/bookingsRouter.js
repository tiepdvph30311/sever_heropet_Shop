...
router.get('/', async (req, res) => {
    try {
        ...
        const defaultFromDate = new Date(today.setHours(0, 0, 0, 0)); // Bắt đầu ngày hiện tại
        const defaultToDate = new Date(today.setHours(23, 59, 59, 999)); // Kết thúc ngày hiện tại

        const filterFromDate = fromDate ? new Date(fromDate) : defaultFromDate;
        const filterToDate = toDate ? new Date(toDate) : defaultToDate;

        // Lọc theo ngày nếu có
        let query = db.collection('CTHDBooking')
                      .orderBy('thoiGianDatLich', 'desc') // Sắp xếp giảm dần theo thời gian
                      .where('trangThai', 'in', ['Chưa xác nhận', 'Đã xác nhận','Đang xử lý']) // Chỉ lấy đơn có trạng thái phù hợp
                      .where('thoiGianDatLich', '>=', filterFromDate)
                      .where('thoiGianDatLich', '<=', filterToDate);
        ...
        res.render('bookings', {
            ...
            currentPage: page,
            totalPages: totalPages,
            fromDate: fromDate || filterFromDate.toISOString().split('T')[0], // Truyền từ ngày vào template
            toDate: toDate || filterToDate.toISOString().split('T')[0],       // Truyền đến ngày vào template
            phoneNumber
        });
    } catch (error) {
        console.error('Error loading bookings:', error);
